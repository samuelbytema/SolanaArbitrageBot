...
impl PumpDex {
    ...
    fn parse_pool(&self, pump_pool: &PumpPool) -> Result<Pool> {
        let base_mint = pump_pool.base_mint.parse::<Pubkey>()?;
        let quote_mint = pump_pool.quote_mint.parse::<Pubkey>()?;
        let pool_address = pump_pool.pool_address.parse::<Pubkey>()?;
        let authority = pump_pool.authority.parse::<Pubkey>()?;
        let program_id = pump_pool.program_id.parse::<Pubkey>()?;

        let base_token = Token::new(
            base_mint,
            "BASE".to_string(),
            "Base Token".to_string(),
            pump_pool.base_decimals,
        );
...
impl DexInterface for PumpDex {
    ...
    async fn get_pools(&self) -> Result<Vec<Pool>> {
        let pools: Vec<PumpPool> = self.make_request("/pools").await?;
        let mut result = Vec::new();
        
        for pool in pools {
            match self.parse_pool(&pool) {
                Ok(parsed_pool) => result.push(parsed_pool),
                Err(e) => tracing::warn!("Failed to parse Pump pool: {}", e),
            }
        }
    ...
    async fn get_pool_state(&self, pool_address: &Pubkey) -> Result<PoolState> {
        ...
        let pool_data: PumpPool = self.make_request(&endpoint).await?;
    ...
    async fn execute_swap(
        ...
        tracing::info!("Executing Pump swap for wallet: {}", wallet);
