...
impl RaydiumDex {
    ...
    fn parse_pool(&self, raydium_pool: &RaydiumPool) -> Result<Pool> {
        let base_mint = raydium_pool.base_mint.parse::<Pubkey>()?;
        let quote_mint = raydium_pool.quote_mint.parse::<Pubkey>()?;
        let pool_address = raydium_pool.pool_address.parse::<Pubkey>()?;
        let authority = raydium_pool.authority.parse::<Pubkey>()?;
        let program_id = raydium_pool.program_id.parse::<Pubkey>()?;

        let base_token = Token::new(
            base_mint,
            "BASE".to_string(), // Will be updated with actual symbol
            "Base Token".to_string(),
            raydium_pool.base_decimals,
        );

        let quote_token = Token::new(
            quote_mint,
            "QUOTE".to_string(), // Will be updated with actual symbol
            "Quote Token".to_string(),
            raydium_pool.quote_decimals,
        );
...
impl DexInterface for RaydiumDex {
    ...
    async fn get_pools(&self) -> Result<Vec<Pool>> {
        let pools: Vec<RaydiumPool> = self.make_request("/pools").await?;
        let mut result = Vec::new();
        
        for pool in pools {
            match self.parse_pool(&pool) {
                Ok(parsed_pool) => result.push(parsed_pool),
                Err(e) => tracing::warn!("Failed to parse Raydium pool: {}", e),
            }
        }
    ...
    async fn get_pool_state(&self, pool_address: &Pubkey) -> Result<PoolState> {
        ...
        let pool_data: RaydiumPool = self.make_request(&endpoint).await?;
        let pool = self.parse_pool(&pool_data)?;
        
        let current_price = pool.get_price(&pool.token_a).unwrap_or(Decimal::ZERO);
        let price_impact = Decimal::ZERO; // Will be calculated based on trade size
        ...
        Ok(PoolState {
            ...
            current_price,
            price_impact,
            volume_24h: pool_data.volume_24h.parse().unwrap_or(Decimal::ZERO),
            tvl,
            apy: None,
        })
    }
    ...
    async fn execute_swap(
        ...
        tracing::info!("Executing Raydium swap for wallet: {}", wallet);
...
// Pool extension methods are now implemented directly in the Pool struct
