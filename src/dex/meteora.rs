...
impl MeteoraDex {
    ...
    fn parse_pool(&self, meteora_pool: &MeteoraPool) -> Result<Pool> {
        let base_mint = meteora_pool.base_mint.parse::<Pubkey>()?;
        let quote_mint = meteora_pool.quote_mint.parse::<Pubkey>()?;
        let pool_address = meteora_pool.pool_address.parse::<Pubkey>()?;
        let authority = meteora_pool.authority.parse::<Pubkey>()?;
        let program_id = meteora_pool.program_id.parse::<Pubkey>()?;

        let base_token = Token::new(
            base_mint,
            "BASE".to_string(),
            "Base Token".to_string(),
            meteora_pool.base_decimals,
        );
        ...
        let quote_token = Token::new(
            ...
            meteora_pool.quote_decimals,
        );

        let reserve_a = meteora_pool.base_reserve.parse::<Decimal>()?;
        let reserve_b = meteora_pool.quote_reserve.parse::<Decimal>()?;
        let fee_rate = meteora_pool.fee_rate.parse::<Decimal>()?;

        Ok(Pool::new(
            meteora_pool.id.clone(),
            DexType::Meteora,
            base_token,
            quote_token,
            pool_address,
...
#[async_trait]
impl DexInterface for MeteoraDex {
    ...
    async fn get_pools(&self) -> Result<Vec<Pool>> {
        let pools: Vec<MeteoraPool> = self.make_request("/pools").await?;
        let mut result = Vec::new();
        
        for pool in pools {
            match self.parse_pool(&pool) {
                Ok(parsed_pool) => result.push(parsed_pool),
                Err(e) => tracing::warn!("Failed to parse Meteora pool: {}", e),
            }
        }
    ...
    async fn get_pool_state(&self, pool_address: &Pubkey) -> Result<PoolState> {
        ...
        let pool_data: MeteoraPool = self.make_request(&endpoint).await?;
    ...
    async fn execute_swap(
        ...
        tracing::info!("Executing Meteora swap for wallet: {}", wallet);
